...
5. **Hub auth + static serving** — myko has no auth hooks. `levi-hub` binds the `CellServer` on `127.0.0.1:<internal>`, and an outer axum server on the public addr: serves dashboard static files, validates the bearer token (query param `?token=` for the CLI transport wrapper; cookie `levi_token` for the browser), and byte-pipes `/myko` WS upgrades to the internal port.
